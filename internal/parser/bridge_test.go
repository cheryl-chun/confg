...
`
	...
	// Test indexed access paths generated by bridge conversion
	feature0, exists := configTree.GetValue("features.0")
	if !exists || feature0 != "ssl" {
		t.Errorf("features.0 = %v, want 'ssl'", feature0)
	}

	server0Host, exists := configTree.GetValue("servers.0.host")
	if !exists || server0Host != "server1.com" {
		t.Errorf("servers.0.host = %v, want 'server1.com'", server0Host)
	}

	server1Port, exists := configTree.GetValue("servers.1.port")
	if !exists || server1Port != 8002 {
		t.Errorf("servers.1.port = %v, want 8002", server1Port)
	}
}
